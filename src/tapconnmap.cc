...
void UprConnMap::manageConnections() {
    LOG(EXTENSION_LOG_INFO, "UprConnMap:: manageConnections start\n");
...
void UprConnMap::addVBConnByVBId(connection_t &conn, int16_t vbid) {
    ...
    if (itr != map_.end()) {
        ...
        if (!found) {
            ...
            LOG(EXTENSION_LOG_INFO,
                "UprConnMap::addVBConnByVBId: producer added for vb %d:", vbid);
        }
    }
    else {
        LOG(EXTENSION_LOG_WARNING,
            "UprConnMap::addVBConnByVBId: producer missing\n");
    }
}
